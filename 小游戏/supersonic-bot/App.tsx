  ...
  return (
    ...
    >
      ...
      {micError && (
        <div className="fixed inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md animate-in fade-in duration-300 z-50">
          <div className="bg-white p-12 rounded-[20px] shadow-2xl flex flex-col items-center w-full max-w-md text-center border text-white rainbow-border-2 [--gradient-angle:270deg] text-[18px]">
            ...
            <div className="py-10">
              <p className="font-medium tracking-tight leading-none mb-2">找不到麦克风</p>
              <p className="font-medium tracking-tight leading-none mb-2">请允许麦克风权限后重试</p>
              <p>{micError}</p>
            </div>
      ...
      {status === GameStatus.GAMEOVER && (
        <div className="fixed inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md animate-in fade-in duration-300 z-50">
          <div className="bg-white p-12 rounded-[20px] shadow-2xl flex flex-col items-center w-full max-w-md text-center border text-white rainbow-border-2 [--gradient-angle:270deg] text-[18px]">
            <h2 className="text-5xl font-medium tracking-tight">游戏结束</h2>
            
            <div className="py-10">
              <p className="text-[96px] font-medium tracking-tight leading-none mb-2">{finalScore.toLocaleString()}</p>
              <p className="text-[#9BA0A6] text-lg tracking-tight font-medium leading-none">分</p>
            </div>
      ...
      {status === GameStatus.WON && (
        <div className="fixed inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md animate-in fade-in duration-300 z-50">
          <div className="bg-white p-12 rounded-[20px] shadow-2xl flex flex-col items-center w-full max-w-md text-center border text-white rainbow-border-2 [--gradient-angle:270deg] text-[18px]">
            <h2 className="text-5xl font-medium tracking-tight">恭喜通关！</h2>
    ...
    <InfoDialog title="用声音飞行" goal="通过用声音的大小编队控制，帮助飞行机器人避开危险，安全抵达终点。" goalNote="需要麦克风权限" onClose={() => {
