...
const App: React.FC = () => {
  ...
  const renderOverlay = () => {
        ...
        return (
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-xl p-[55px]">
                <div className="bg-black text-white px-[30px] lg:px-10 py-10 lg:py-[50px] rounded-3xl text-center w-full rainbow-border animate-rotate-gradient animate-in fade-in zoom-in duration-200 flex flex-col items-center min-w-xs max-w-md">
                    <h2 className="text-[48px] font-medium mb-6 leading-none tracking-tight">好球！</h2>
                    
                    <div className="flex flex-col items-center mb-10">
                        <p className="text-lg tracking-tight font-medium leading-none flex gap-1.5">{gameState.strokes} 杆
                        {scoreInfo && (
                    ...
                    {ENABLE_AI_CADDY && (
                        <div className="bg-[#202020] p-5 rounded-2xl mb-10 gap-3 flex flex-col justify-center w-full min-h-[100px]">
                            <p className="text-lg text-[#9BA0A6] tracking-tight font-medium">球童建议</p>
                            {gameState.commentaryLoading ? (
                            <div className="flex justify-center py-2">
                                <Loader2 className="animate-spin text-white" />
                            </div>
                            ) : (
                            <p className="text-lg font-medium text-white tracking-tight">"{gameState.lastCommentary || "干得好！"}"</p>
                            )}
                        </div>
                    )}
    ...
    if (gameState.state === 'GAME_OVER') {
        ...
        const scoreText = parDiff === 0 ? "平标准杆" : (parDiff > 0 ? `高出标准杆 ${parDiff} 杆` : `低于标准杆 ${Math.abs(parDiff)} 杆`);
        
        return (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-xl p-[55px]">
            <div className="bg-black text-white px-[30px] lg:px-10 py-10 lg:py-[50px] rounded-3xl text-center w-full rainbow-border animate-rotate-gradient animate-in fade-in zoom-in duration-200 flex flex-col items-center min-w-xs max-w-md">
                <h2 className="text-[48px] font-medium mb-6 leading-none tracking-tight">全场景通关</h2>
                ...
                {ENABLE_AI_CADDY && (
                    <div className="bg-[#202020] p-5 rounded-2xl mb-10 gap-3 flex flex-col justify-center w-full min-h-[100px]">
                        <p className="text-lg text-[#9BA0A6] tracking-tight font-medium">最终分析</p>
                        {gameState.commentaryLoading ? (
                            <div className="flex justify-center py-2">
                                <Loader2 className="animate-spin text-white" />
                            </div>
                        ) : (
                            <p className="text-lg font-medium text-white tracking-tight">"{gameState.lastCommentary || "多么精彩的一局比赛！"}"</p>
                        )}
                    </div>
                )}
  ...
  const getScoreTerm = (strokes: number, par: number) => {
    ...
    if (diff === 0) return { term: "保帕", color: "text-black" };
  ...
  return (
    ...
    <div className="w-full h-[calc(100%-100px)] bg-black font-sans overflow-hidden relative rounded-2xl">
      ...
      <InfoDialog title="你能一杆进洞吗？" goal="瞄准，尽情挥杆！小心障碍物，并从你的私人球童那里获得有用的提示。" onClose={() => {}} />
    </div>
